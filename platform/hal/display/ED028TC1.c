...
#define EINK_WAIT() \
        ...
        while ((GPIO_PinRead(BOARD_EINK_BUSY_GPIO, BOARD_EINK_BUSY_GPIO_PIN) == 0) && (--_timeout)) { _delay_ms(1); } \
        if (_timeout == 0)  return(EinkTimeout)
...
/**
 * @brief Clear desired window - make it white
 ...
 */
EinkStatus_e
EinkClearWindow (uint16_t X, uint16_t Y, uint16_t W, uint16_t H)
{
 ...
 */ 
void
EinkEnableFrontlight (bool enable)
{
