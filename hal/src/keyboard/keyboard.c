...
int kbd_read_key( kbd_event_t* key )
{
    ...
    for (uint8_t i = 0; i < events_count; i++){
        ...
        key->event = (retval & 0x80) >> 7; // key release/pressed is stored on the last bit
    }
    // Clear all interrupts
    int err = TCA8418_SendByte(i2c_inst, REG_INT_STAT, 0xFF);
    return (err != kStatus_Success )?(-EIO):(0);
}