...
int tar_catalog(struct tar_ctx *ctx,const char *path, const char *sanitized)
{
    if (sanitized == NULL) {
        return 0;
    }
...
int un_tar_file(struct tar_ctx *ctx, mtar_header_t *header, const char *where)
{
    int ret              = 0;
    int bytes_read       = 0;
    ssize_t yet_to_write = header->size;
    ...
    {
         int read_size = yet_to_write > ctx->size ? ctx->size : yet_to_write;
         memset(ctx->buffer, 0, ctx->size);
